var jsPsychImageFeedbackTask = (function (jspsych) {
    ...
    class ImageFeedbackTaskPlugin {
        ...
        createCanvas(display_element) {
            ...
            canvas.style.backgroundColor = 'green'; // Set the background color to green
        ...
        //                     doneButton.style.display = 'block'; // Show the "Next -->" button
        ...
        runFeedbackPhase(display_element, trial, data, image_clicks, tapOrder) {
            ...
            // Show all GIFs/images at the same time
            tapOrder.forEach((index) => {
                ...
                gifElement.style.left = `${x + ctx.canvas.getBoundingClientRect().left}px`; // Use canvas's left position
                gifElement.style.top = `${y + ctx.canvas.getBoundingClientRect().top}px`; // Use canvas's top position
